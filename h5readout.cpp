...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        mtype.insertMember( FRAGMENT_DATA_EVENT_ID,              HOFFSET(FragmentData, event_id             ), H5::PredType::NATIVE_LONG);
        mtype.insertMember( FRAGMENT_DATA_TIMESTAMP,             HOFFSET(FragmentData, timestamp            ), H5::PredType::NATIVE_DOUBLE);
        mtype.insertMember( FRAGMENT_DATA_COARSE_TIME,           HOFFSET(FragmentData, coarse_time          ), H5::PredType::NATIVE_LONG);
        mtype.insertMember( FRAGMENT_DATA_ENERGY,                HOFFSET(FragmentData, energy               ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_TRACE_LENGTH,          HOFFSET(FragmentData, trace_length         ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_CRATE_ID,              HOFFSET(FragmentData, crate_id             ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_SLOT_ID,               HOFFSET(FragmentData, slot_id              ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_CHANNEL_ID,            HOFFSET(FragmentData, channel_id           ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_TIME_HIGH,             HOFFSET(FragmentData, time_high            ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_TIME_LOW,              HOFFSET(FragmentData, time_low             ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_TIME_CFD,              HOFFSET(FragmentData, time_cfd             ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_FINISH_CODE,           HOFFSET(FragmentData, finish_code          ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_CHANNEL_LENGTH,        HOFFSET(FragmentData, channel_length       ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_CHANNEL_HEADER_LENGTH, HOFFSET(FragmentData, channel_header_length), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_OVERFLOW_CODE,         HOFFSET(FragmentData, overflow_code        ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_CFD_TRIG_SOURCE_BIT,   HOFFSET(FragmentData, cfd_trig_source_bit  ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_CFD_FAIL_BIT,          HOFFSET(FragmentData, cfd_fail_bit         ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_MODMSPS,               HOFFSET(FragmentData, adc_frequency        ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_HW_REVISION,           HOFFSET(FragmentData, hardware_revision    ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_ADC_RESOLUTION,        HOFFSET(FragmentData, adc_resolution       ), H5::PredType::NATIVE_INT);
        mtype.insertMember( FRAGMENT_DATA_ADC_OVER_UNDER_FLOW,   HOFFSET(FragmentData, adc_over_underflow   ), H5::PredType::NATIVE_SHORT);
        ...
        while (event_id < max_evt_cnt && (pItem = data_source->getItem())) {
            ...
            process_item(event_id++, dynamic_cast<CPhysicsEventItem&>(*castableItem), pfragdata);
        }
        ...
        fprintf(stderr, "Failed to read and export event stream t0o HDF5.\n");
        return EXIT_FAILURE;
    }
